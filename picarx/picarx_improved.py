...
try:
    ...
    #from sim_robot_hat import utils
...
class Picarx(object):
    ...
    def __init__(self, 
        ...
        # ensure we stop if program shutdown
        atexit.register(self.stop)

    @log_on_start(logging.DEBUG, "Setting Motor {motor} to {speed}") 
    def set_motor_speed(self, motor, speed):
        ...
        logging.log(logging.DEBUG, f"Set {'left' if motor else 'right'} motor to {direction} @ {speed}")
    ...
    @log_on_start(logging.DEBUG, "Setting servo angle: {value}")
    ...
    @log_on_start(logging.DEBUG, "set_power: {speed}")
    ...
    def backward(self, speed):
        ...
        if current_angle != 0:
            ...
            r = 4 / math.tan(math.radians(abs_current_angle)) # turn radius in inchs
            power_scale = (r-2)/(r+2)
            logging.log(logging.DEBUG, f"backward:Power Scaling:{power_scale}, turning rad:{r}")
    ...
    def forward(self, speed):
        ...
        if current_angle != 0:
            ...
            r = 4 / math.tan(math.radians(abs_current_angle)) # turn radius in inchs
            power_scale = (r-2)/(r+2)
            logging.log(logging.DEBUG, f"forward:Power Scaling:{power_scale}, turning rad:{r}")
            if (current_angle / abs_current_angle) > 0:
                self.set_motor_speed(1, 1*speed * power_scale)
                self.set_motor_speed(2, -speed) 
            else:
    ...
    @log_on_start(logging.INFO, "Stopping Motors")
    def stop(self):
        '''
        Execute twice to make sure it stops
        '''
        #logging.log(logging.INFO, "In Stop Function")
        for _ in range(2):
